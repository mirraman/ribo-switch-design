"""
main.py — CLI entry points for riboswitch sequence design and verification.

Commands:
    ribo-design   Design riboswitch sequences via NSGA-II.
    ribo-verify   Fold a sequence and compare it against target structures.
"""
...
@click.option(
    '--bp-distance-obj/--no-bp-distance-obj',
    default=True,
    show_default=True,
    help=(
        'Include base-pair distance to S_ON as a 4th NSGA-II objective so '
        'the optimizer selects sequences whose MFE structure matches S_ON. '
        'Use --no-bp-distance-obj to restore the original 3-objective mode.'
    ),
)
@click.option(
    '--require-exact-on',
    ...
    default=False,
    help=(
        'After optimization, filter the Pareto front to candidates whose MFE '
        'structure exactly matches S_ON (bp_dist_on == 0). '
        'Falls back to the full front with a warning if the filter leaves nothing.'
    ),
)
def design(
    structure1, structure2, input_file, output,
    population, generations, mutation_rate, seed, verbose, top_k,
    bp_distance_obj, require_exact_on,
):
    ...
    Examples:
        ...
        ribo-design "(((...)))" "...((...))" --require-exact-on -v
    """
    # Get structures
    if input_file:
        try:
    ...
    def progress_callback(gen, front):
        if verbose and (gen % 10 == 0 or gen == generations - 1):
            ...
            click.echo(
                f"  Gen {gen:4d}: {summary['count']:3d} solutions, "
                f"mean gap = {summary.get('gap_on_mean', 0)/100:.2f} + "
                f"{summary.get('gap_off_mean', 0)/100:.2f}  "
                f"exact_ON={exact_on}"
            )
            last_report[0] = gen
    ...
    pareto_front = nsga2(
        ...
        include_structure_objective=bp_distance_obj,
    )
    ...
    # Optional post-filter: keep only sequences whose MFE == S_ON exactly
    if require_exact_on:
        filtered = filter_by_structure(pareto_front, max_bp_dist_on=0)
        if filtered:
            click.echo(
                f"  --require-exact-on: kept {len(filtered)} / {len(pareto_front)} candidates "
                f"(bp_dist_on == 0)"
            )
            pareto_front = filtered
        else:
            click.echo(
                "  Warning: --require-exact-on found no exact matches; "
                "showing full Pareto front instead.",
                err=True,
            )
...
def verify(sequence: str, struct_on: str, struct_off: str | None):
    """Fold a sequence and compare its MFE structure to target ON/OFF shapes.

    SEQUENCE   RNA sequence (A/C/G/U, e.g. GGGAAAUUCCCC).

    STRUCT_ON  ON-state target structure in dot-bracket notation.

    STRUCT_OFF (optional) OFF-state target structure in dot-bracket notation.
               When provided, verification is run against both targets.

    Examples:

        ribo-verify GGGAAAUUCCCC "((((....))))"
    ...
    if len(seq_clean) != len(struct_on):
        click.echo(
            f"Error: sequence length {len(seq_clean)} != structure length {len(struct_on)}",
            err=True,
        )
        sys.exit(1)
    if struct_off is not None and len(seq_clean) != len(struct_off):
        click.echo(
            f"Error: sequence length {len(seq_clean)} != S_OFF length {len(struct_off)}",
            err=True,
        )
        sys.exit(1)
...
def main():
    """Entry point for ribo-design."""
    design()


def main_verify():
    """Entry point for ribo-verify."""
    verify()
