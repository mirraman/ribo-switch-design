...
if USING_RUST:
    ...
    def evaluate_candidate(seq: Sequence, s_on: Structure, s_off: Structure, params: TurnerParams) -> tuple[int, int, int, str]:
        return _ribo_rs.evaluate_candidate(_seq_bytes(seq), s_on.pair_table, s_off.pair_table)

    def evaluate_batch(seqs: Iterable[Sequence], s_on: Structure, s_off: Structure, params: TurnerParams) -> list[tuple[int, int, int, str]]:
