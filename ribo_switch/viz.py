...
def pareto_front_data(candidates: list) -> dict:
    data = {'gap_on': [], 'gap_off': [], 'stability': [], 'rank': [], 'sequence': []}
    for c in candidates:
        if hasattr(c, 'gap_on'):
            data['gap_on'].append(c.gap_on / 100.0)
...
def convergence_data(history: list[list]) -> dict:
    data = {'generation': [], 'front_size': [], 'mean_gap_on': [], 'mean_gap_off': [], 'min_gap_on': [], 'min_gap_off': [], 'mean_combined': []}
    for gen, front in enumerate(history):
        ...
        data['mean_combined'].append(sum((g1 + g2 for g1, g2 in zip(gaps_on, gaps_off))) / len(gaps_on))
    return data
...
def component_diversity_data(candidates: list, graph) -> dict:
    ...
    return {'component_index': list(range(n_components)), 'component_size': [len(comp.nodes) for comp in graph.components], 'unique_assignments': [len(variants) for variants in component_variants]}
...
def format_pareto_table(candidates: list, max_rows: int=10) -> str:
    ...
    lines = []
    header = f'{'#':>3} {'Gap_ON':>8} {'Gap_OFF':>8} {'bpD_ON':>7} {'bpD_OFF':>7} {'Stability':>10} {'Sequence':>20}'
    lines.append(header)
    ...
    for i, c in enumerate(sorted_cands[:max_rows]):
        ...
        lines.append(f'{i + 1:>3} {gap_on:>8.2f} {gap_off:>8.2f} {bp_on_str} {bp_off_str} {stability:>10.2f} {seq:>20}')
...
def export_pareto_json(candidates: list, filepath: str) -> None:
    ...
    for i in range(len(data['sequence'])):
        records.append({'sequence': data['sequence'][i], 'gap_on_kcal': data['gap_on'][i], 'gap_off_kcal': data['gap_off'][i], 'stability_kcal': data['stability'][i], 'pareto_rank': data['rank'][i]})
...
def try_matplotlib_plot(candidates: list, output_path: str=None) -> bool:
    ...
    scatter = ax.scatter(data['gap_on'], data['gap_off'], c=data['stability'], cmap='viridis_r', alpha=0.7, s=50)
