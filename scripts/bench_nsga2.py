"""Wall-clock benchmark for nsga2 at a few (len, pop, gen) sizes.

Run with: python scripts/bench_nsga2.py
"""
import time
import random
import cProfile
...
def make_pair(length: int) -> tuple[str, str]:
    on, off = CASES[length]
    assert len(on) == length and len(off) == length, (
        f"len mismatch for {length}: on={len(on)} off={len(off)}"
    )
    return on, off
...
def run(length: int, pop: int, gens: int) -> float:
    ...
    print(
        f"  len={length:3d}  pop={pop:4d}  gen={gens:4d}  "
        f"time={dt:7.3f}s  |front|={len(front)}"
    )
    return dt
